...
namespace Builder
{
    ...
    {
        std::cout << "\nPASS 1: Verifying '" << m_config.ELEMENTS_TABLE_NAME << "' tables..." << std::endl;
        ...
        for (const auto& entry : fs::directory_iterator(targetPath))
        {
            ...
            std::string query = "SELECT * FROM \"" + m_config.ELEMENTS_TABLE_NAME + "\";";
            sqlite3_stmt* stmt;
            ...
            for (int i = 0; i < colCount; ++i)
            {
                ...
                if (colName == m_config.ELEMENT_ID_COLUMN) elemIdIdx = i;
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                if (m_verifiedElements.count(currentElemId))
                {
                    if (currentProps != m_verifiedElements.at(currentElemId))
                    {
                ...
                {
                    m_verifiedElements[currentElemId] = currentProps;
                }
            }
        ...
        std::cout << "Verification successful. Found " << m_verifiedElements.size() << " unique elements." << std::endl;
        return true;
    }
    ...
    void EnvelopeBuilder::EnvelopeDataInMemory(const fs::path& targetPath)
    {
        ...
        for (const auto& entry : fs::directory_iterator(targetPath))
        {
            ...
            {
                if (tableName == m_config.ELEMENTS_TABLE_NAME) continue;
                ...
                while (sqlite3_step(stmt) == SQLITE_ROW)
                {
                    ...
                    if (m_verifiedElements.find(elementId) == m_verifiedElements.end()) continue;
                    ...
                    for (int i = 0; i < colCount; ++i)
                    {
                        ...
                        if (colName == m_config.ELEMENT_ID_COLUMN || colName == m_config.SET_N_COLUMN || colName == m_config.ELEM_TYPE_COLUMN) continue;
                        ...
                        if (colType == SQLITE_INTEGER || colType == SQLITE_FLOAT)
                        {
                            ...
                            if (currentValue < 0)
                            {
                                std::cerr << "Error in" << elementId << " element. The reinforcement vaalue is negative!" << std::endl;
                            }

                            currentRowNumerics[colName] = currentValue;

                            if (_envelopedData[elementId].find(colName) == _envelopedData[elementId].end() || currentValue > _envelopedData[elementId][colName])
                            {
                                _envelopedData[elementId][colName] = currentValue;
                            }
                        }
                    }

                    // Step 2: If it's a shell, additionally calculate and envelop the sums
                    bool isShell = m_verifiedElements.at(elementId).count(m_config.ELEM_TYPE_COLUMN) &&
                        m_verifiedElements.at(elementId).at(m_config.ELEM_TYPE_COLUMN) == "2";
                    ...
                    if (isShell)
                    {
                        ...
                        if (_envelopedData[elementId].find(sum_i_key) == _envelopedData[elementId].end() || sum_i > _envelopedData[elementId][sum_i_key])
                        {
                            _envelopedData[elementId][sum_i_key] = sum_i;
                        }

                        const std::string sum_j_key = "__Asw_sum_j";

                        if (_envelopedData[elementId].find(sum_j_key) == _envelopedData[elementId].end() || sum_j > _envelopedData[elementId][sum_j_key])
                        {
                            _envelopedData[elementId][sum_j_key] = sum_j;
                        }
                    }
                }
                sqlite3_finalize(stmt);
    ...
    {
        const std::string dbFilename = createSummedVersion ? m_config.OUTPUT_DB_SUMMED_FILENAME : m_config.OUTPUT_DB_FILENAME;
        ...
        "b1", "h1", "a1", "a2", "t1", "t2", "reinfStep1", "reinfStep2", "a3", "a4", "sl1", "sl2") 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);)";
        ...
        std::vector<std::string> propOrder = {
            ...
            "a1", "a2", "t1", "t2", "reinfStep1", "reinfStep2", "a3", "a4", "sl1", "sl2"
        };

        for (const auto& pair : m_verifiedElements)
        {
        ...
        else
        {
            ...
            createReinfTableSql << "CREATE TABLE \"" << m_config.ENVELOPED_TABLE_NAME << "\" ("
                << "\"" << m_config.SET_N_COLUMN << "\" INT, "
                << "\"" << m_config.ELEMENT_ID_COLUMN << "\" INT, "
                << "\"" << m_config.ELEM_TYPE_COLUMN << "\" INT";

            for (const auto& header : finalHeaders) createReinfTableSql << ", \"" << header << "\" REAL";

            createReinfTableSql << ", PRIMARY KEY(\"" << m_config.ELEMENT_ID_COLUMN << "\")"
                << ", CONSTRAINT \"fk_elements\" FOREIGN KEY(\"" << m_config.ELEMENT_ID_COLUMN << "\") REFERENCES \"" << m_config.ELEMENTS_TABLE_NAME << "\"(elemId));";

            sqlite3_exec(finalDbHandle, createReinfTableSql.str().c_str(), 0, 0, &errMsg);

            std::stringstream insertReinfSql;
            insertReinfSql << "INSERT INTO \"" << m_config.ENVELOPED_TABLE_NAME << "\" (\"" << m_config.SET_N_COLUMN << "\", \"" << m_config.ELEMENT_ID_COLUMN << "\", \"" << m_config.ELEM_TYPE_COLUMN << "\"";
            ...
            for (const auto& elemPair : _envelopedData)
            {
                ...
                if (m_verifiedElements.count(elementId) && m_verifiedElements.at(elementId).count(m_config.ELEM_TYPE_COLUMN))
                    sqlite3_bind_int(insertStmt, 3, std::stoi(m_verifiedElements.at(elementId).at(m_config.ELEM_TYPE_COLUMN)));
                else
                    sqlite3_bind_null(insertStmt, 3);

                bool isShellForSumming = createSummedVersion &&
                    m_verifiedElements.count(elementId) &&
                    m_verifiedElements.at(elementId).count(m_config.ELEM_TYPE_COLUMN) &&
                    m_verifiedElements.at(elementId).at(m_config.ELEM_TYPE_COLUMN) == "2";
