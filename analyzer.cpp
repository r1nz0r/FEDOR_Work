...
void saveFinalResults(sqlite3* temp_db_handle) {
    ...
    sqlite3_open(OUTPUT_DB_FILENAME.c_str(), &final_db_handle);
...
int main() {
    ...
    if (fs::exists(TEMP_DB_FILENAME)) {
        fs::remove(TEMP_DB_FILENAME);
    }
    sqlite3* temp_db_handle;
    if (sqlite3_open(TEMP_DB_FILENAME.c_str(), &temp_db_handle) != SQLITE_OK) {
